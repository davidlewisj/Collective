...
| Org membership: single org (`entity_main`), demo directory seeded only where dev-login is allowed (prod starts empty), `COLLECTIVE_BOOTSTRAP_ADMIN` → active org_admin, all other new sign-ins `pending` and content-blocked until an admin approves, in-app Directory (approve/deny) | §2.6.1 (#11), §8.1 D18 | ✅ tested (bootstrap provision/promote, pending 403 gate, `/users` exclusion, approve→active, deny removes + revokes sessions) |
